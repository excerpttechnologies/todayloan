...
// seedPersonalLoanBanks.js
// One-time script: replaces the entire bank list with ONLY the active (uncommented) banks below.
// Any bank in the DB that is NOT in this list will be REMOVED.
// Run with:  node seedPersonalLoanBanks.js
...
// ✅ Only the ACTIVE banks (previously-commented-out entries removed completely)
const BANKS_TO_SEED = [
  { bankName: 'ADITYA BIRLA', dsaCode: 'DAUMUM03410' },
  { bankName: 'AXIS BANK', dsaCode: 'DSA12594MUM' },
  { bankName: 'Bajaj Finserve (Prime)', dsaCode: '199445' },
...
const bankNamesToKeep = BANKS_TO_SEED.map(b => b.bankName);
...
async function run() {
  ...
  // 1. Remove EVERY bank currently in DB that is NOT in our active list
  const delOthers = await Bank.deleteMany({ bankName: { $nin: bankNamesToKeep } });
  console.log(`🧹 Removed ${delOthers.deletedCount} bank(s) not present in the active list`);

  // 2. Delete existing entries that ARE in the seed list (to avoid duplicates before re-inserting fresh copies)
  const delResult = await Bank.deleteMany({ bankName: { $in: bankNamesToKeep } });
  console.log(`🗑️  Removed ${delResult.deletedCount} old bank(s) from the active list (will re-insert fresh)`);

  // 3. Insert the fresh, active-only list
  const inserted = await Bank.insertMany(BANKS_TO_SEED);
  console.log(`✅ Inserted ${inserted.length} banks (out of ${BANKS_TO_SEED.length})`);

  // 4. Verify final state
  const finalCount = await Bank.countDocuments();
  console.log(`📊 Total banks in DB now: ${finalCount}`);
  console.log(`📋 Active banks present: ${await Bank.countDocuments({ bankName: { $in: bankNamesToKeep } })}`);
