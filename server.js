...
//     directives: {
//       ...helmet.contentSecurityPolicy.getDefaultDirectives(),
//       "frame-ancestors": ["'self'", "http://localhost:3000"],//3000
//     },
//   },
// }));
// app.use('/uploads', (req, res, next) => {
...
//   next();
// });
// app.use(cors({ origin: process.env.CLIENT_URL || 'http://localhost:3000', credentials: true })); //3000
// app.use(morgan('dev'));
// app.use(express.json({ limit: '50mb' }));
// app.use(express.urlencoded({ extended: true, limit: '50mb' }));
...
//   skipSuccessfulRequests: true   // 🔥 success ಆದ logins count ಆಗಲ್ಲ
// });
// app.use('/api/v1/auth', authLimiter);
// app.use('/api/v1', limiter);
...
// React Router fallback — LAST route
app.get("*", (req, res) => {
  res.sendFile(path.join(__dirname, "dist", "index.html"));
});
...
// Error handler (only ONE, at the very end)
app.use((err, req, res, next) => {
  console.error(err.stack);
  res.status(500).json({ message: err.message || 'Server error' });
});